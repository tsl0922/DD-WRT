...
static char* get_apcliif(int idx)
{
	if (idx == 0)
		return "apcli0";
	else
...
//station
	...
	if (nvram_nmatch("apsta", "wl%d_mode", idx) || nvram_nmatch("apstawet", "wl%d_mode", idx)) {
		...
		if (nvram_nmatch("psk", "wl%d_akm", idx)
		    || nvram_nmatch("psk2", "wl%d_akm", idx)
		...
		log_eval("ap_client", "ra0", get_apcliif(idx), essid, key);
	} else {
...
void init_network(int idx)
{
	...
	if (isSTA(idx)) {
		...
		if (startradius[idx]) {
			if (idx == 0)
				log_eval("rt2860apd", "-i", "ra0");
			else
				log_eval("rt2860apd", "-i", "ba0");
...
void stop_wifi()
{
	...
	eval("ifconfig", "wdsx0", "down");
	eval("ifconfig", "wdsx1", "down");
	eval("ifconfig", "wdsx2", "down");
	eval("ifconfig", "wdsx3", "down");
	eval("ifconfig", "wdsx4", "down");
	eval("ifconfig", "wdsx5", "down");
	eval("ifconfig", "wdsx6", "down");
	eval("ifconfig", "wdsx7", "down");
